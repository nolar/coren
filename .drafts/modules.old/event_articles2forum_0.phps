...
// Еще надо сделать заброс комментариев (notes) из тех же артиклов в форум в качестве ответов на топики.
// Или все же сделать разными модулями? articles2forum & notes2forum. Что гораздо правильнее по концепции, но
// не очень - по реализации (придется копировать код и использовать общую таблицу в двух модулях).

// А еще нужно проверить можно ли по грантам совать эту новость/статью/событие в форум,
...
// проконтроллировать всех посетителей форума, куда мы отправить запись. Вообще не можем, даже по идее.
// И как тут обойтись - неизвестно. Проверять, наверное, что категория записи должна быть общей, иначе
// просто не отправлять в форум.

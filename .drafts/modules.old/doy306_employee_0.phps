...
//todo: 1. деление по department: отделам, возрастным группам.
...
class doy306_employee_0_exception_duplicate extends list_0_exception_duplicate {}

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

class doy306_employee_0 extends list_0 implements dbaware
{
...
protected function get_default_fields ($entity, $parent)
{
	switch ($entity)
	{
		...
		case 'employee':
			...
			'department'		=> null, 'department_'	=> null,
			'position'		=> null,
			'qualify'		=> null,
...
protected function guess_item_fake ($entity, $parent, $submit)
{
	if (
	    ...
	    (!isset($submit['department'   ]) || ($submit['department'   ] == '')) &&
	    (!isset($submit['position'     ]) || ($submit['position'     ] == '')) &&
	    (!isset($submit['qualify'      ]) || ($submit['qualify'      ] == '')) &&
	    (!isset($submit['birthday'     ]) || ($submit['birthday'     ] == '')) &&
...
protected function get_guessed_itemid ($entity, $parent, $data)
{
	...
	{
		case 'department':
			$result = core::find_scalar(array($data), array('department', 'id'), null);
			break;

		case 'employee':
			$result = core::find_scalar(array($data), array('employee', 'id'), null);
			break;
...
protected function get_guessed_action ($entity, $parent, $data)
{
	...
	{
		case 'department':
			$itemid =        core::find_scalar(array($data), array('department', 'id'), null) != '';
		...
		case 'employee':
			$itemid =        core::find_scalar(array($data), array('employee', 'id'), null) != '';
			$delete = (bool) core::find_scalar(array($data), array('delete'        ), null);
			$fake = $this->guess_item_fake($entity, $parent, $data);
			if (!$itemid && !$delete) $result = $fake ? null : 'append'; else
			if (!$itemid &&  $delete) $result =         null           ; else
...
{//returns: array(entity, action, submit, filter, parent, itemid, child);
	...
	if (!in_array($entity, array('employee', 'department'))) $entity = null;
...
protected function do_read_list ($entity, $filter, $parent, $itemid, &$meta)
{
	switch ($entity)
	{
		case 'department':
			...
			$count = core::db('select_department_count', compact('parent', 'itemid'));
			$skip    = $offset  = 0;
			$size    = $count - $skip;
			$page    = $pagemin = $pagemax = 1;
			$sorting = $reverse = null;
			$meta = compact('count', 'page', 'size', 'skip', 'offset', 'pagemin', 'pagemax', 'sorting', 'reverse');
			$items = core::db('select_department_data', compact('parent', 'itemid', 'page', 'size', 'skip', 'count', 'offset', 'pagemin', 'pagemax', 'sorting', 'reverse'));
			return $items;
		...
		case 'employee':
			...
			$count = core::db('select_employee_count', compact('parent', 'itemid'));
			...
			$meta = compact('count', 'page', 'size', 'skip', 'offset', 'pagemin', 'pagemax', 'sorting', 'reverse');
			$items = core::db('select_employee_data', compact('parent', 'itemid', 'page', 'size', 'skip', 'count', 'offset', 'pagemin', 'pagemax', 'sorting', 'reverse'));

			$ids_department = array();
			foreach ($items as $key => $item)
			{
				if (isset($item['department'])) $ids_department[] = $item['department'];
			}
			$data_department = core::db('select_department_info', array('itemids'=>array_unique($ids_department)));
			foreach ($items as $key => $item)
			{
				if (isset($item['department']) && array_key_exists($item['department'], $data_department)) $items[$key]['department_'] = $data_department[$item['department']];
			}
...
protected function do_action ($entity, $action, $itemid, $itemnew, $itemold, &$newitemid)
{
	switch ($entity)
	{
		case 'department':
			...
			{
				case 'append':
					$newitemid = core::db('insert_department', compact('itemnew'));
					break;

				case 'modify':
					core::db('update_department', compact('itemid', 'itemnew')); 
					break;

				case 'remove':
					core::db('delete_department', compact('itemid'));
					break;
		...
		case 'employee':
			switch ($action)
			{
				...
				case 'remove':
					core::db('delete_employee', compact('itemid'));
					if ($itemold['photo_storage'] != '') core::call($this->photo_storage_module, 'unlink', array('id'=>$itemold['photo_storage']));
					break;
